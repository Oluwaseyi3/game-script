...
import {
    ...
    BATTLE_ROYALE_BUY_IN,        // Add this to constants (e.g. 0.2 SOL)
...
import { sleep, getLatestBlockhash } from "../../utils.js";
...
export interface BattleRoyaleState {
    ...
    players: {
        ...
        exitTx: string | null;
        buyInTx?: string | null;  // Added field to store buy-in transaction
    }[];
    prizePool: number;
    winners: {
        wallet: string;
 ...
 */
async function verifyBuyInTransaction(
    signature: string,
    playerWallet: string
...
async function initBattleRoyale() {
    ...
    const battleRoyaleState: BattleRoyaleState = {
        isActive: true,
        tournamentId,
        tokenMint: null,
        poolId: null,
...
export async function registerPlayer(
    playerWallet: string,
    transactionSignature: string  // New parameter to receive transaction signature
    ...
    const playerExists = brState.players.some((p: any) => p.wallet === playerWallet);
    if (playerExists) {
...
export async function recordPlayerExit(playerWallet: string, exitTx: string): Promise<{ success: boolean, message: string }> {
    const brState = await readBattleRoyaleState();
    ...
    // Find player
    const playerIndex = brState.players.findIndex((p: any) => p.wallet === playerWallet);
    if (playerIndex === -1) {
...
async function calculateWinners(rugpullTime: number): Promise<BattleRoyaleState["winners"]> {
    const brState = await readBattleRoyaleState();

    // Filter players who exited before rugpull
    const successfulPlayers = brState.players.filter((p: any) =>
    ...
    successfulPlayers.sort((a: any, b: any) => {
    ...
    const diamondWinners = successfulPlayers.filter((p: any) => (p.exitTime || 0) >= diamondCutoff);
    ...
    const platinumWinners = successfulPlayers.filter(
        (p: any) => (p.exitTime || 0) >= platinumCutoff && (p.exitTime || 0) < diamondCutoff
    );
    ...
    const goldWinners = successfulPlayers.filter(
        (p: any) => (p.exitTime || 0) >= goldCutoff && (p.exitTime || 0) < platinumCutoff
    );

    // Silver Nerves (all other successful exits): 10% of the pool
    const silverWinners = successfulPlayers.filter((p: any) => (p.exitTime || 0) < goldCutoff);
    ...
    if (diamondWinners.length > 0) {
        ...
        diamondWinners.forEach((player: any) => {
            winners.push({
    ...
    if (platinumWinners.length > 0) {
        ...
        platinumWinners.forEach((player: any) => {
            winners.push({
                wallet: player.wallet,
                tier: "Platinum",
    ...
    if (goldWinners.length > 0) {
        ...
        goldWinners.forEach((player: any) => {
            winners.push({
                wallet: player.wallet,
                tier: "Gold",
    ...
    if (silverWinners.length > 0) {
        ...
        silverWinners.forEach((player: any) => {
            winners.push({
                wallet: player.wallet,
                tier: "Silver",
...
export async function runBattleRoyale() {
    ...
    try {
        ...
        if (!poolId || !positionId || !txId) throw new Error("Invalid pool creation result.");
...
export async function checkPlayerRegistration(tournamentId: string, wallet: string): Promise<{
    success: boolean;
    registered: boolean;
    hasExited: boolean;
}> {
    try {
        ...
        // Find player
        const player = brState.players.find((p: any) => p.wallet === wallet);
...
export async function getLeaderboard(): Promise<{
    ...
    try {
        ...
        if (brState.isActive) {
            ...
            // Tournament has ended - show final results
            // Sort winners by their reward tier and then by how close they got to the rugpull
            type TierKey = "Diamond" | "Platinum" | "Gold" | "Silver";
...
export async function forceEndTournament(adminKey: string): Promise<{ success: boolean, message: string }> {
    // Verify admin key
