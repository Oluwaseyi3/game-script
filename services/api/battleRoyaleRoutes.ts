...
async function verifyExitTransaction(
    signature: string,
    playerWallet: string,
    tokenMint: string
        ...
        for (const preBalance of preTokenBalances) {
            if (preBalance.mint === tokenMint &&
                ...
                const postBalance = postTokenBalances.find(
                    post => post.mint === tokenMint && post.owner === playerWallet
                );
...
router.post('/start', async (req, res) => {
    try {
        ...
        runBattleRoyale().catch(err =>
            console.error('[API] Error starting Battle Royale:', err)
        );
...
router.get('/leaderboard', async (req, res) => {
    try {
        ...
        // Define interface for the state objects
        interface Winner {
            wallet: string;
            tier: string;
            reward: number;
        ...
        // Define proper types for leaderboard items
        type WinnerLeaderboardItem = {
            rank: number;
            wallet: string;
            tier: string;
            reward: number;
        ...
        let leaderboard: WinnerLeaderboardItem[] | ActiveLeaderboardItem[] = [];

        if (!brState.isActive && brState.winners.length > 0) {
            // Tournament ended with winners
            leaderboard = brState.winners
                .sort((a: Winner, b: Winner) => a.exitTimeToPull - b.exitTimeToPull)
            ...
            leaderboard = brState.players
                .filter((p: Player) => p.exitTime !== null)
                .sort((a: Player, b: Player) => (b.exitTime ?? 0) - (a.exitTime ?? 0))
